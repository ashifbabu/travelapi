...
@router.post(
    ...
    description="Cancel an order using the BDFare API.",
)
async def order_cancel(payload: OrderCancelRequest):
    """
    Cancel an order using BDFare API.

    Args:
        payload (OrderCancelRequest): The request payload containing the order reference.

    Returns:
        OrderCancelResponse: The response from the BDFare API.
    """
    ...
    try:
        ...
        return OrderCancelResponse(success=True, data=response_data)
