...
class OrderRetrieveRequest(BaseModel):
...
@router.post(
    "/bookingRetrieve",
    response_model=OrderRetrieveResponse,
    summary="Retrieve Order Details",
    description="Retrieve order details using the BDFare API.",
)
async def retrieve_order(payload: OrderRetrieveRequest):
    """
    Retrieve order details using BDFare API.

    Args:
        payload (OrderRetrieveRequest): The request payload containing the order reference.

    Returns:
        OrderRetrieveResponse: The response from the BDFare API.
    """
    ...
    try:
        ...
        return OrderRetrieveResponse(success=True, data=response_data)
