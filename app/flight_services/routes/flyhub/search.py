...
async def generate_token():
    """
    Authenticate with FlyHub API and cache the token.
    """
    global cached_token
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            cached_token["expires_at"] = time.time() + 3600  # Cache the token for 1 hour
            return token_data["token"]
        else:
            raise HTTPException(status_code=response.status_code, detail="Failed to authenticate with FlyHub.")
    except httpx.RequestError as exc:
        raise HTTPException(status_code=500, detail=f"Error during token generation: {exc}")
...
async def search_flights(payload: dict):
    ...
    # Generate a new token if it is expired or not set
